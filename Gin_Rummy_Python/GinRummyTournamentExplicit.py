...
csvFileX = open('csvP_explicit_hand_X.csv', 'w', newline='')
out = csvmodule.writer(csvFileX)
csvFileY = open('csvP_explicit_hand_Y.csv', 'w', newline='')
    ...
    while scores[0] < GinRummyUtil.GOAL_SCORE and scores[1] < GinRummyUtil.GOAL_SCORE:
        ...
        if (toCSV1):
            ...
            for turn in csvTurnRecords:
                ...
                matrix = [[0 for c in range(91)] for r in range(4)] #All info of the turn
                handOutcome = 0
                if (csv[0] == 0):
                            ...
                            if (i == 12):
                                ...
                                if (i == 3): # if melds
                                    ...
                                    if i == 6 or i == 7:
                                        if csv[i]:
                                            for r in range(4):
                                                for c in range(rankOffset, rankOffset+13):
                                                    matrix[r][c] = 1
                                    elif (csv[i] != None):
                                        card = csv[i]
                                        suit = card.getSuit()
...
playVerbose = False
match("SimpleGinRummyPlayer", "SimpleGinRummyPlayer2", 10000)
